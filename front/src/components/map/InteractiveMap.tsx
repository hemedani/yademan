...
const InteractiveMap: React.FC<InteractiveMapProps> = ({ onLoad }) => {
  ...
  const setShowCategoryList = useMapStore((state) => state.setShowCategoryList);
    ...
    try {
      ...
      if (!bounds.isEmpty()) {
        map.current.fitBounds(bounds, {
          ...
          padding: {
            top: 180,
            bottom: 270, // Increased padding for bottom to account for timeline
            left: 120,
            right: 120,
          },
          maxZoom: 15,
      ...
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-[132px] right-4 z-10">
          <button
            ...
            title={showCategoryList ? "پنهان کردن دسته‌بندی‌ها" : "نمایش دسته‌بندی‌ها"}
            className={`w-12 h-12 rounded-lg backdrop-blur-md border shadow-lg flex items-center justify-center transition-all duration-300 ${
              showCategoryList
                ? "bg-[#FF007A]/20 border-[#FF007A] shadow-[0_0_10px_rgba(255,0,122,0.4)]"
                : "bg-black/60 border-[#333] hover:border-[#FF007A]/50 hover:shadow-[0_0_15px_#FF007A]"
            }`}
          >
            <svg
              className={`w-5 h-5 transition-colors duration-300 ${
                showCategoryList ? "text-[#FF007A]" : "text-[#a0a0a0]"
              }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
        ...
        {/* Stats indicator positioned just below toggle button */}
